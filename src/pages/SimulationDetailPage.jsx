  ...
  useEffect(() => {
    const fetchSimulation = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/v1/simulator/${id}`, {
        ...
        if (response.ok) {
          ...
          setError('No se pudo cargar la información de esta simulación.');
        }
      } catch (err) {
      ...
      <main className="flex-1 p-10 overflow-y-auto">
        <header className="mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium mt-1">Generada el {new Date(result.fecha_simulacion).toLocaleDateString('es-PE')} • {result.ifi_seleccionada || 'Cálculo Genérico'}</p>
          </div>
        </header>
